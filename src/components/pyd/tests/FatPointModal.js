...
import { uiCloseFatPointModal, uiCloseModal, uiClosePanelModal, uiClosePointModal } from '../../../actions/ui';
...
import { clearActiveFatPoint, fatPointUpdate } from '../../../actions/fat';
...
export const FatPointModal = () => {
    ...
    const [formValues, setFormValues] = useState( initFatPoint );
    ...
    const { step, category, description, nonConformity } = formValues;
    ...
    const { fatPointModalOpen } = useSelector(state => state.ui)

    // Extraemos el state.calendar del Store y nos quedamos con el fat point activo
    const { index, ...active } = useSelector(state => state.fat.active)

    const prueba = useSelector( state => state.fat.active)
    ...
    useEffect(() => {
        ...
        if ( active ) {
            ...
            setFormValues( initFatPoint );
        }
               
    }, [active, setFormValues]); // Establecemos como dependencias lo que cambia dentro
    
    // Función que se disparará cuando se quiera cerrar el modal
    const closeModal = () => {

        // Cerramos el modal poniendo la variable del store [ui] en false
        dispatch( uiCloseFatPointModal() );

        // Limpiamos el fat point activo del store
        dispatch( clearActiveFatPoint() );
    ...
    const handleSubmitForm = ( e ) => {
        ...
        // // Si tenemos un fat point activo quiere decir que estamos editando, si es null es uno nuevo
        // if ( active ) {
        ...
        dispatch( fatPointUpdate( active, index ));
    ...
    return (
        ...
        <Modal
            isOpen={ fatPointModalOpen } // Muestra u oculta el modal
        ...
        >
            ...
            <h1> { (active) ? active.title : 'Edición del Punto de Inspección' } </h1>
            <hr />
            <form 
                className="container"
            ...
            >   
                ...
                <div className="form-group">
                    ...
                    <input 
                        ...
                        onChange={ handleInputChange }
                    />
                    <small id="emailHelp" className="form-text text-muted">Introduce la disconformidad</small>
                </div>
